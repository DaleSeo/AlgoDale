...
그러므로 우리는 문자열 안에 각 글자가 몇 번씩 나왔는지 [해시 테이블(Hash Table)](/data-structures/hash-table/) 자료 구조에 저장을 해야할 것입니다.
...
입력 문자열의 길이를 `n`이라고 했을 때, 공간 복잡도는 해시 테이블이 차지하는 메모리에 좌우가 될 텐데요.
문자열에 영어 대문자만 들어있기 때문에 해시 테이블의 크기는 26을 넘을 수 없습니다.
따라서 공간 복잡도는 `O(26) = O(1)`이 됩니다.

반면에 시간 복잡도는 두 개의 포인터를 사용하여 루프를 도는 `O(n)`의 시간이 걸리고, 가장 많이 들어있는 글자의 수를 찾는데는 `O(26)`의 시간이 걸려서, `O(26n) = O(n)`이 되겠습니다.
