...
정수 배열 `nums`와 정수 `k`가 주어졌을 때, `nums`에서 가장 자주 나오는 `k`개의 요소를 찾아라.
요소의 순서는 중요하지 않다.
...
첫 번째 예제를 기준으로 생각을 해보면 해시 테이블이 이와 같은 모양이 될 것입니다.
