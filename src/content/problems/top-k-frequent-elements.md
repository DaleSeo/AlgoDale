...
정렬이나 힙을 쓰지 않고 이 문제를 풀 수는 없을까요?
그러면 성능을 `O(n)`까지 개선할 수 있을 것 같은데요.

이차원 [배열(array)](/data-structures/array/) 자료구조를 활용해보면 어떨까요?
...
입력 배열에 한 가지 숫자만 채워져 있다면, 그 숫자의 빈도는 `n`일 것입니다.
