...
그리고 카펫에 있는 총 격자의 개수는 카펫의 너비와 높이를 곱한 값, 즉 카펫의 너비와 일치할 것입니다.
...
하지만 이러한 방식으로 여러 쌍의 카펫의 너비와 높이가 나올 수 있습니다.
...
위 풀이에서는 카펫의 너비와 노란색 부분의 너비를 이용하여 문제를 풀었는데요.
이번에는 카펫의 둘레를 이용해서도 풀어보면 어덜까요?
...
우리는 카펫이 높이를 3부터 둘레까지 증가시키면서 카펫의 너비를 찾을 수 있을 것입니다.
...
def solution(brown, yellow):
    ...
    for height in range(3, half_peri):
...
카펫의 둘레를 `p`이라고 했을 때 이 풀이의 시간 복잡도는 `O(0.5p) = O(p)`가 됩니다.
공간 복잡도는 정해진 개수의 변수 외에는 추가 메모리를 사용하지 않기 때문에 `O(1)`입니다.

## 풀이 3

이전 두 개의 풀이 모두 카펫의 너비와 높이를 선형 탐색으로 찾고 있는데요.
만약에 이분 탐색으로 찾을 수 있다면 성능을 향상시킬 수 있을 것입니다.

이분 탐색을 수행하려면 중앙을 기준으로 언제 앞 부분을 탐색하고 언제 뒷 부분을 탐색할지를 판단할 수 있어야 하는데요.
...
- 카펫의 높이와 너비의 곱이 카펫의 넓이보다 같다면, 원하는 카펫의 높이를 찾아다는 뜻입니다. 따라서, 탐색을 종료할 수 있습니다.
