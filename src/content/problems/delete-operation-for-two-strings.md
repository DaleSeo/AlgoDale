...
예를 들어, `"a"`와 `"ab"`를 똑같게 만들기 위해서는 두번째 문자열에서 `"b"` 하나만 삭제하면 되죠?
...
마찬가지로 두번째 문자열에서 두 번째 문자까지 고려하면 삭제해야 할 글자의 수는 `2`가 됩니다.

```
"" vs "ab" => 2
```

이 번에는 첫번째 문자열에 첫번째 문자를 추가하고, 두 번째 문자열에 빈 문자열로 놓아보면 삭제해야 할 글자의 수는 `1`이 됩니다.
