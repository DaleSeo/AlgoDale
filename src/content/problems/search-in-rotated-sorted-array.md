...
원래 기본 이분 탐색에서는 조건문이 두 갈래로 갈리죠?
검색 범위의 중앙에 위치한 값이 찾으려는 값보다 작으면 좌측 절반을 탐색하고, 크면 우측 절반을 탐색합니다.

회전되어 있는 배열에서 이분 탐색을 할 때는 고려할 것이 좀 더 생겨서 조건문이 네 갈래로 갈립니다.
...
간단합니다! 좌측의 맨 처음에 있는 값보다 중앙에 위치한 값이 더 크거나 같으면 좌측이 정렬되어 있다고 판단할 수 있습니다.
